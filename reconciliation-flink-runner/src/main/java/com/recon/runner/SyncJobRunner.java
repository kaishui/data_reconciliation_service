 ...
 *
 * Reads job configuration from a JSON file and builds/submits the Flink CDC sync job.
 *
 * Usage:
 *   java -jar reconciliation-flink-runner.jar job-config.json
 *
 * Or submit to a Flink cluster:
 *   flink run -c com.recon.runner.SyncJobRunner reconciliation-flink-runner.jar --config job-config.json
 */
